...
Архітектура самого ME/CSME (окремий x86-CPU, власна ізольована SRAM,
мікроядро на базі Minix, власні ring0/ring3, незалежність від стану
живлення хоста) тепер `source-confirmed` напряму проти офіційного
білого паперу Intel — див. `hardware/CSME-ARCHITECTURE.md`. Вміст
самого ME-регіону цієї плати лишається нерозібраним.

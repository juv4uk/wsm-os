...
### Підтверджено без змін (WINDOWS-OBSERVED, живий повторний запит)

BIOS `F22e` (AMI, 2018-03-09); материнська плата Gigabyte H170-Gaming 3;
CPU Intel Core i5-6400 @ 2.70GHz, 4 ядра/4 логічні/без SMT; RAM 2×8 ГіБ
DDR4-2133 (`CT8G4DFS8213.C8FBD1`, `TEAMGROUP-UD4-2133`); GPU Intel HD
Graphics 530 (драйвер `26.20.100.7262`) і NVIDIA GTX 1050 Ti (CIM
`DriverVersion` `32.0.15.8266`, що розшифровується як той самий `582.66`,
уже задокументований — незалежно перехресно підтверджено рідним
`nvidia-smi` нижче, не лише повторно зчитано з CIM); накопичувачі:
Kingston `SNV2S1000G` 1ТБ NVMe, Samsung `HD321KJ` 320ГБ, Samsung 850 EVO
120ГБ, усі `Status: OK`; мережа: Killer E2200 Gigabit + Tailscale Tunnel;
білд ОС `10.0.26200`.

### Досі щиро UNKNOWN, перепідтверджено

`Confirm-SecureBootUEFI` досі відмовляє наживо з тією самою помилкою,
що й 2026-08-29: `Unable to set proper privileges. Access was denied.`
Це не нова інформація — та сама межа доступу, перепідтверджена сьогодні,
а не прийнята на віру як незмінна.

### Реальні зміни з 2026-08-29

Це перші дві прогалини можливостей, які оригінальний профіль явно
позначив як такі, що потребують вирішення до M1 — обидві тепер закриті,
підтверджені наживо, не просто повторно заявлені:

1. **GPU тепер видно з Linux/WSL, не лише з Windows.**
   `nvidia-smi` (`/usr/lib/wsl/lib/nvidia-smi`) тепер успішно запускається:
   драйвер `582.66`, CUDA runtime `13.0`, GTX 1050 Ti повідомляє живу
   телеметрію (37°C, P8 неактивний, 588 МіБ використано на момент
   знімка — це знімок, не апаратний факт). `nvcc`
   (`/usr/local/cuda-12.6/bin/nvcc`) теж тепер у PATH. Оригінальний
   документ казав, що NVML не вдалося ініціалізувати, а `nvcc`
   відсутній; обидва тепер доступні.
2. **Повний bare-metal тулчейн тепер встановлений і в PATH через Guix**:
   `clang`, `lld`, `nasm`, `xorriso`, `objcopy` — усі резолвяться
   (раніше "не знайдено в поточному PATH агента"). Ціль
   `x86_64-unknown-none` для Rust встановлена (раніше "не встановлена";
   `rustup target list --installed` тепер також показує
   `x86_64-unknown-linux-gnu`, `x86_64-pc-windows-gnu`,
   `wasm32-unknown-unknown`). `qemu-system-x86_64` 10.2.1 доступний
   через Guix (власний `manifest.scm` цього репозиторію); раніше було
   "не знайдено в поточному PATH агента".

### Новий доказ, якого не було в оригінальному знімку

- `lspci` (встановлений через `guix shell pciutils`) підтверджує, що
  WSL2 показує Linux лише віртуалізовані PCI-пристрої: virtio SCSI
  консоль, virtio файлову систему та Microsoft "Basic Render Driver"
  3D-контролер — жодного реального Intel/NVIDIA PCI ID звідси не видно
  взагалі. Це прямо підтверджує наявне архітектурне рішення, що робота
  з нативними драйверами дисплея/PCI не може розроблятись чи
  тестуватись з цього WSL-процесу агента проти реального кремнію
  GPU/NIC; QEMU (або окремий прохід реального заліза) лишається єдиним
  шляхом.
- **Прискорення KVM для QEMU — реальне, зараз чинне обмеження, не факт
  про Windows-залізо.** `/dev/kvm` існує, але WSL-юзер агента не в групі
  `kvm`; реальна, відтворена помилка: `qemu-system-x86_64: -accel kvm:
  Could not access KVM kernel module: Permission denied`. Цей хост має
  повний `sudo (ALL : ALL) ALL`, але sudo тут вимагає інтерактивного
  пароля, якого в агента нема і який він не буде намагатись отримати чи
  обійти — див. `docs/QEMU-SETUP.md` для повного опису. Це не стосується
  фізичної машини як такої; це факт дозволів WSL/Linux-юзера, записаний
  тут, бо він прямо впливає на те, що зараз означає "запустити під
  QEMU" у цьому середовищі.

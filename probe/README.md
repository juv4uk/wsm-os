...
**Оновлення, 2026-09-03: третя реальність більше не `?`** —
`physical-boot-probe.c` реально запущено на фізичному i5-6400 власника
через Ventoy, див. "Реальний результат на фізичному залізі" нижче.
Жива ревізія мікрокоду, що читається як `0x1`
...
**Застереження, сказане прямо, перед фактом**: жива ревізія мікрокоду й
TSC-темп у цій пробі досі читатимуться так само, як під QEMU/TCG,
*якщо* запуск відбудеться під QEMU — але на реальному залізі
очікується, що вони покажуть інші, справжні значення (ревізія
мікрокоду — ймовірно `0xD6` чи `0xC2`, TSC-темп — реальну частоту
i5-6400, не темп емуляції TCG). Саме це різницю ця перша фізична проба
й має показати.

## Реальний результат на фізичному залізі (2026-09-03) — перший LIVE/PHYSICAL цього репозиторію

Власник реально виконав кроки вище на своєму GA-H170-Gaming 3 /
i5-6400 через Ventoy. Повний, дослівний вивід `WSM-PROBE-LOG.TXT`
(перекодовано з UTF-16LE, як записано пробою, у UTF-8 для читабельності
тут — байти самого файлу не змінені):

```text
=== wsm-os physical-boot-probe: real CPU/platform state ===

CR0            = 0x0000000080000013  (PE=1 PG=1)
CR3            = 0x000000004F2F3000  (page table base)
CR4            = 0x0000000000000668  (PAE=1 PGE=0)
EFER (MSR 0xC0000080) = 0x0000000000000500  (LME=1 LMA=1 NXE=0)
RSP            = 0x000000004F5F3660  (16-byte aligned: yes)
RFLAGS         = 0x0000000000200202  (IF=1, interrupts ENABLED)
GDTR           = base 0x0000000000000000 limit 0x0000000000000047
IDTR           = base 0x000000004F5F0000 limit 0x0000000000000FFF
CPUID leaf 0   = max_leaf=22 vendor=GenuineIntel
CPUID leaf 1   = signature=0x00000000000506E3 (family 6 model 94 stepping 3) initial_APIC_ID=0
Live microcode revision (RDMSR 0x8B, read directly, pre-OS) = 0x00000000000000C2
TSC (RDTSC)    = t1=0x00006D35F8EF0F96 t2=0x00006D360919D882 delta=271239404 cycles / 100ms Stall()
  implied rate = 2712 cycles/us (2712 MHz-equivalent)
Memory map     = 46 descriptors, 4227091 total pages (16512 MiB), 4039737 conventional/usable pages (15780 MiB)
ACPI RSDP      = present
  address      = 0x000000005A506000
SMBIOS         = present (SMBIOS3/64-bit)
  address      = 0x000000005AD9F000
Framebuffer    = 0x00000000E1000000  1024x768  pixels-per-scanline=1024  mode=1
```

`LIVE-CONFIRMED`, справжнє фізичне залізо — не QEMU/TCG. Це заповнює
третю з "трьох реальностей", яку власник назвав після першого прогону
цієї проби: STATIC-образ прошивки, LIVE/VIRTUAL-машина, і тепер уперше
**LIVE/PHYSICAL-машина**, яка досі була навмисно залишена `?`.

**Найточніша й найважливіша знахідка**: **жива ревізія мікрокоду на
реальному залізі — `0xC2`**, точно та сама ревізія, яку
`FIT-AND-STRUCTURE-ANALYSIS.md` знайшов `STATIC-CONFIRMED` усередині
самого образу прошивки F22e (запис FIT типу 1, дата 2017-11-16),
перехресно підтверджена другим незалежним інструментом. Це перше пряме
емпіричне підтвердження того, що BIOS/UEFI-фаза завантаження реально
використовує власний вбудований мікрокод прошивки — до того, як
Windows (за раніше зафіксованим значенням реєстру `0xD6`,
`wsm/research/handoff-state.md`) накладає свій новіший. Ланцюг тепер
має три реально підтверджені точки, не дві: `F22e-прошивка (0xC2,
STATIC) -> UEFI до OS (0xC2, тепер LIVE/PHYSICAL) -> Windows після
завантаження (0xD6, LIVE/PHYSICAL за реєстром)`.

**Реальні розбіжності з QEMU/TCG, варті фіксації, не проігноровані**:

- **`CPUID max_leaf=22` (0x16) на реальному залізі проти `13` (0xD) під
  QEMU з `-cpu Skylake-Client-v1`.** Реальний кремній підтримує більше
  листків CPUID, ніж модель QEMU відтворює — реальна, задокументована
  межа точності апаратного налаштування QEMU з `docs/QEMU-SETUP.md`,
  не помилка тієї роботи, а її чесно виявлена межа.
- **`EFER NXE=0` на реальному залізі проти `NXE=1` під OVMF.** У цій
  точній точці завантаження реальна прошивка AMI Aptio (F22e) ще не
  увімкнула No-Execute, тоді як OVMF це вже зробив до того самого
  моменту виконання проби. Позначено як спостережена розбіжність, не
  пояснена до кінця тут — не варто домислювати причину без окремої
  перевірки.
- **TSC: `2712 MHz-еквівалент` реальний проти `~2699-2723
  MHz-еквівалент`, що плавав між запусками під TCG.** Це вперше
  справжнє вимірювання, не темп емуляції — і воно впритул збігається з
  паспортною базовою частотою i5-6400 (2.70 ГГц), як і мало бути.
  Раніше застереження в цьому документі (не читати TCG-числа як
  підтвердження реальної частоти) підтвердилось: TCG-числа плавали
  між прогонами (2699, 2712, 2718, 2723 МГц у різних запусках цієї
  сесії), тоді як реальний вимір — один, стабільний, і саме він тепер
  `LIVE-CONFIRMED` для фізичної частоти.
- **RAM: `16512 MiB` загалом (46 дескрипторів пам'яті) — незалежно
  узгоджується** з уже записаним `docs/OWNER-HARDWARE-PROFILE.md` ("RAM
  2×8 ГіБ" = 16 ГіБ), тепер підтверджено іншим методом (UEFI
  `GetMemoryMap` до завантаження ОС, не Windows-інструмент).
- **Framebuffer: реальна Intel HD Graphics 530** (`0xE1000000`,
  1024x768) — зовсім інша адреса й режим, ніж віртуальний GPU QEMU
  (`0x80000000`, 1280x800), як і очікувалось.

## Друге фізичне завдання: чи ввімкнено Intel Boot Guard у режимі примусової перевірки

Наступний крок — власник хоче вставити власний DXE-модуль напряму в
саму прошивку F22e. Перш ніж будувати це, треба знати: чи чип реально
відхилить модифікований (непідписаний) образ на апаратному рівні, до
того, як BIOS взагалі почне виконуватись.

**Реальна знахідка в самому образі F22e** (перевірено напряму власним
парсером FIT, не здогадка): у FIT-таблиці присутні і Key Manifest
(`type 0x0b`), і Boot Policy Manifest (`type 0x0c`) — інфраструктура
Boot Guard реально є в цій прошивці. **Але це ще не каже, чи ввімкнено
примусову перевірку на конкретному чипі власника** — це рішення
прошивається окремо, в one-time запобіжниках (FPF) всередині ME під
час виробництва, незалежно від вмісту файлу прошивки.

**Помилка, знайдена й виправлена перед тим, як вона встигла зашкодити**:
спочатку припущено, що інструмент CHIPSEC має готовий модуль
`common.bootguard` для перевірки цього на живій машині. Перевірено
напряму — склоновано `chipsec/chipsec` і прогреповано весь код: жодної
згадки `bootguard`, `acm`, `key_manifest`, `fpf` чи "verified boot" ніде
немає. Це неправильне, неперевірене твердження вчасно відкинуто, не
дозволено дійти до реалізації.

**Реальний, перевірений шлях, натомість**: `intelmetool` (з проєкту
coreboot) читає стан Boot Guard через один MSR — `0x13A`
(`BOOTGUARD_SACM_INFO`). Точний розклад бітів узятий напряму з
реального вихідного коду coreboot
(`util/intelmetool/intelmetool.c`), не з переказу — переказ із
вторинного джерела, знайдений раніше, дав **інший, неправильний**
розклад бітів для того самого MSR, і це розходження впіймано саме
звіркою з першоджерелом:

```text
біт 0     nem_enabled     (Cache-As-RAM активовано ACM)
біти 1-2  tpm_type
біт 3     tpm_success
біт 4     facb_fpf
біт 5     measured_boot
біт 6     verified_boot   -- критичний біт для рішення про модифікацію
біт 7     module_revoked
біт 32    btg_capability  (чи Boot Guard узагалі присутній як можливість)
```

Це додано напряму в `physical-boot-probe.c` (`read_and_print_bootguard()`)
— жодного нового Linux/DKMS-ризику, той самий, уже перевірений
UEFI-шлях. Під QEMU/TCG (де цей MSR не змодельований) проба чисто
повідомляє `btg_capability=0`, без краху — очікувана, коректна обробка
відсутності апаратної підтримки в емуляції, не помилка.

### Реальний результат (2026-09-03) — Boot Guard відсутній як можливість на цьому чипі

Власник реально запустив оновлену пробу на GA-H170-Gaming 3.
`WSM-PROBE-LOG1.TXT`, дослівно:

```text
Boot Guard MSR (0x13A, BOOTGUARD_SACM_INFO) = 0x0000000000000000
  btg_capability=0 -- Boot Guard NOT present as a capability on this chip.
```

**`LIVE-CONFIRMED`, вирішальний результат.** `btg_capability=0` —
сильніше за просто "verified_boot вимкнено": сам біт можливості Boot
Guard не встановлений узагалі, тобто відповідний запобіжник (FPF),
найімовірніше, ніколи не був спалений виробником на цій конкретній
платі. Узгоджується з уже відомим: консюмерська модель, "Standard
Manageability: No" (`hardware/CSME-ARCHITECTURE.md`) — Gigabyte,
схоже, ніколи не вмикала цю можливість на цій лінійці плат, попри те,
що сама прошивка несе Key Manifest і Boot Policy Manifest
(`FIT-AND-STRUCTURE-ANALYSIS.md`) — наявність KM/BPM у файлі й
реальний стан запобіжника виявились справді різними фактами, як і
передбачалось.

**Практичний висновок**: немає апаратного ACM-рівня перевірки, що
відхилить модифікований образ прошивки на цьому чипі. Шлях до вставки
власного DXE-модуля відкритий для продовження — не заблокований на
рівні CPU.

Той самий прогін ще раз незалежно підтвердив стабільність попередніх
знахідок цієї сесії, не одноразовий збіг: жива ревізія мікрокоду знову
`0xC2`, TSC знову `2712 MHz-еквівалент`, `CPUID max_leaf=22`, RAM
`16512 MiB` — та сама поведінка, другий реальний запуск.

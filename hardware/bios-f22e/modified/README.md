...
**Статус: перша спроба прошивки через Q-Flash на реальному залізі
провалилась з `invalid BIOS image` — не через нашу модифікацію
структури (вона повністю коректна, перевірено нижче), а через окрему,
власну перевірку Gigabyte (`$BDR`). Зроблено обхідний патч одного
байта, ще НЕ перевірено на залізі.** Деталі — розділ "$BDR" нижче.

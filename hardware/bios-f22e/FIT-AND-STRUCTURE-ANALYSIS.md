...
**Оновлення, 2026-09-03 — третя точка ланцюга, тепер теж
`LIVE-CONFIRMED`, не лише дві.** `wsm-os/probe/physical-boot-probe.c`
реально прочитав живу ревізію мікрокоду напряму на цьому залізі, до
завантаження Windows (той самий MSR `IA32_BIOS_SIGN_ID`, прочитаний на
фазі UEFI, не з реєстру ОС): результат — **`0xC2`**, точно збігається
з `STATIC-CONFIRMED` значенням, вбудованим у сам образ F22e. Повний
ланцюг тепер підтверджений у трьох точках, не виведений із двох:

```text
F22e-прошивка (0xC2, STATIC-CONFIRMED, знайдено в FIT)
        |
        v
UEFI до завантаження ОС (0xC2, LIVE-CONFIRMED, physical-boot-probe.c)
        |
        v
Windows після завантаження (0xD6, LIVE-CONFIRMED, реєстр)
```

Це перше пряме емпіричне підтвердження, що між увімкненням живлення й
запуском ОС прошивка справді використовує власний вбудований мікрокод
— не лише логічний висновок із порівняння дат. Джерело: `wsm-os/probe/
README.md`.

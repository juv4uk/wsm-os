...
**Статус: source-confirmed для загальної архітектури CSME-родини;
`not-yet-verified` для вмісту ME-регіону саме цієї плати.** Розширює
Фазу 1 `docs/POWER-ON-SEQUENCE.md` і закриває частину `not-yet-verified`
з `hardware/bios-f22e/FIT-AND-STRUCTURE-ANALYSIS.md` (розділ 1) —
конкретно: *що* таке ME/CSME структурно, ще не *що саме* лежить у
2 093 056 байтах ME-регіону цієї конкретної плати, який лишається
нерозібраним.
